...
In order to keep the host-facing API consistent across platforms and enforce the
message passing semantics, all messages are serialized, sent across the boundary,
then deserialized using [Bincode](https://docs.rs/bincode). Type definitions and
serialization code for the foreign (non-Rust) languages are generated by
